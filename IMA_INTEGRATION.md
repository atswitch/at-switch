...
**当前验收状态：** 功能实现和自动化测试不等同于真机完成。macOS 原生凭据读取和
只读契约检查已成功；实际返回的 `sub_model_infos` 为对象形状，适配器已修正并由
单元测试覆盖。真实切换验收已两次进入新增自定义模型步骤，但 ima 服务返回频率限制
`100003`，因此未写入测试模型；此前该类失败后的远端首选、本地选择和运行状态已
自动恢复并回读确认。钥匙串授权恢复后，真实账号曾成功完成一次切换。随后往返验收
发现该账号存在“原首选为空”的场景：ima 删除受管理模型后不会把首选恢复为空，补偿
重建模型又返回 `100006`，因此该场景停在待恢复记录，不能宣称精确恢复已通过。
适配器会保留加密恢复记录并明确报错，不会静默替换为其他模型。Windows 仅完成官方
包静态研究与独立模块编译检查，未进行系统凭据读取或切换恢复的真机验收。

当前本地门禁：前端 106 项测试、Rust 176 项测试通过；3 项显式 opt-in 的真实环境
测试不计为通过。生产前端构建、Rust 格式、所有 target 的 Clippy、依赖许可证检查
以及 macOS 调试 `.app` 打包已通过。调试包仅使用 ad-hoc 签名，不代表正式签名、
公证或可发布版本的权限体验已验收。
...
当前恢复流程先恢复原先非空的远端首选，再删除本次接管创建的受管理模型，回读两个
场景后恢复本地受控字段。删除失败不能掩盖为清理完成；远端恢复失败时保留基线和
可重试状态。若原模型已被用户删除，应报告原配置失效，不能默默替换为另一个模型。

**无首选恢复需要单独验证：** 客户端未发现清除远端首选的调用，也未发现将
`model_id` 置空的用例。不能假定空字符串就是删除首选，也不能把设置 `is_default`
项当作精确恢复缺失状态。当前实现要求空首选场景仍指向本次受管理模型，删除该模型
后检查远端首选是否恢复为空；不满足预期则进入补偿与待恢复状态。这条路径已有
模拟测试，尚不能据此声称真实服务能够精确恢复空首选，必须完成服务端回读验收。
...
- [ ] 真实接口模型 ID 关联、两个场景首选读取与恢复。
- [ ] 原首选缺失时的精确恢复，或在修改前明确拒绝该状态。
- [ ] 官方/原始模型 → 第三方 A → 第三方 B → 原始模型 → 第三方 A。
- [ ] 重复切换不增加多余模型项，原有用户自定义模型保持不变。
- [ ] 安全退出与重启后两个入口默认模型正确，旧会话行为明确。
